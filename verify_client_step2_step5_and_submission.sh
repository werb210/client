...
async function getCatalog() {
  ...
  // Fallback: legacy → normalize minimal fields needed for tests
...
(async () => {
  ...
  console.log(`✅ Loaded ${products.length} products (${schema.length} canonical fields)`);
  ...
  console.log('\n🧪 SUBMISSION API probe (tries common endpoints)…');
  const endpoints = [
  ...
  for (const ep of endpoints) {
    try {
      ...
      console.log(`${ep.m} ${ep.u} → ${r.status}`);
      if (String(r.status).startsWith('2')) { ok = true; break; }
    } catch (e) {
      console.log(`${ep.m} ${ep.u} → error (${e.message})`);
    }
  }
  if (!ok) console.log('ℹ️ No known Submission API accepted; confirm the correct endpoint when available.');

  console.log('\n✅ CLIENT Step 2 & Step 5 smoke tests completed.');
})();
JS
