...
export function useLenders(params?: { q?: string; country?: string; active?: boolean }) {
  ...
  useEffect(()=>{ let on=true; (async()=>{ /* ensure products fetched */  try{ const rows=await listLenders(params); if(on) setData(rows); }catch(e:any){ if(on) setErr(e);} })(); const products = await fetchProducts();
